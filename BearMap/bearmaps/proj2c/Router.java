...
public class Router {
    ...
    public static List<Long> shortestPath(AugmentedStreetMapGraph g, double stlon, double stlat,
        ...
        return new WeirdSolver<>(g, src, dest, 30).solution();
     ...
     * route.
     * @source https://github.com/chenyuxiang0425/cs61b_sp19/blob/master/proj2c/bearmaps/proj2c/Router.java
     */
    public static List<NavigationDirection> routeDirections(AugmentedStreetMapGraph g, List<Long> route) {
        ...
        List<WeightedEdge<Long>> ways = getNeighbors(g, route);
        if (ways.size() == 1) {
            NavigationDirection naviDir = setNavigation(currDirection, ways.get(0).getName(), ways.get(0).weight());
            directionList.add(naviDir);
        }else {
            for (int i = 1; i < ways.size(); i++) {
                WeightedEdge<Long> prevEdge = ways.get(i-1);
                WeightedEdge<Long> nextEdge = ways.get(i);
                Long preVertex = prevEdge.from();
                Long currVertex = prevEdge.to();
                Long nextVertex = nextEdge.to();

                String prevName = prevEdge.getName() != null ? prevEdge.getName(): "unknown road";
                String nextName = nextEdge.getName() != null ? nextEdge.getName(): "unknown road";
                // update distance
                currEdgeDistance += prevEdge.weight();

                // which means the user keep moving
                if (!prevName.equals(nextName)) {
                    double prevBearing = NavigationDirection.bearing(getLon(g, preVertex), getLon(g,currVertex), getLat(g, preVertex), getLat(g, currVertex));
                    double nextBearing = NavigationDirection.bearing(getLon(g, currVertex), getLon(g,nextVertex), getLat(g, currVertex), getLat(g, nextVertex));
                    int newDir = NavigationDirection.getDirection(prevBearing, nextBearing);

                    // do not use newDir, cuz we same the prior direction
                    NavigationDirection nav = setNavigation(currDirection, prevName, currEdgeDistance);
                    directionList.add(nav);
                    // update the direction
                    currDirection = newDir;
                    // reset the distance (cuz we change the direction, distance on current road is 0)
                    currEdgeDistance = 0;
                }
                if (i == ways.size() - 1) {
                    currEdgeDistance += nextEdge.weight();
                    NavigationDirection nav = setNavigation(currDirection, nextName, currEdgeDistance);
                    directionList.add(nav);
                }
            }
        }
    ...
    // get the next available route
    private static double getLon(AugmentedStreetMapGraph g, Long vertex) {
        Map<Long, Node> getPos = g.getPosition();
        return getPos.get(vertex).lon();
    }
    private static double getLat(AugmentedStreetMapGraph g, Long vertex) {
        Map<Long, Node> getPos = g.getPosition();
        return getPos.get(vertex).lat();
    }

    private static List<WeightedEdge<Long>> getNeighbors(AugmentedStreetMapGraph g, List<Long> route) {
        List<WeightedEdge<Long>> neighbors = new ArrayList<>();
        for (int i = 1; i < route.size(); i++) {
            // from v to w
            long v = route.get(i - 1);
            long w = route.get(i);
            for (WeightedEdge edge: g.neighbors(v)) {
                if (edge.to().equals(w)) {
                    neighbors.add(edge);
                }
            }
    ...
    private static NavigationDirection setNavigation(int dir, String name, double distance) {
